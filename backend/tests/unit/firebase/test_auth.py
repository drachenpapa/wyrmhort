...
def test_valid_token_returns_uid(monkeypatch):
    ...
    uid = get_current_user_uid(_make_request("valid-token"))
    assert uid == "user-123"


def test_missing_auth_header_raises_401():
    with pytest.raises(HTTPException) as exc_info:
        get_current_user_uid(_make_request(None))
...
def test_invalid_token_raises_401(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc_info:
        get_current_user_uid(_make_request("bad-token"))
    assert exc_info.value.status_code == 401


def test_disallowed_email_raises_403(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc_info:
        get_current_user_uid(_make_request("valid-token"))
